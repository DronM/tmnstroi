 ...
 */
function DocMovementList_View(id,options){	

	options.HEAD_TITLE = "Перемещение материалов";

	DocMovementList_View.superclass.constructor.call(this,id,options);
	
	var model = options.models.DocMovementList_Model;
	var contr = new DocMovement_Controller();
	...
	var filters = options.fromApp? null:{
		...
		,"oktmo":{
			"binding":new CommandBinding({
				"control":new OKTMOEdit(id+":filter-ctrl-oktmo",{
					...
					"labelCaption":"ОКТМО отправитель:",
		...
		,"oktmo_contract":{
			"binding":new CommandBinding({
				"control":new OKTMOContractEdit(id+":filter-ctrl-oktmo_contract",{
					"contClassName":"form-group-filter",
					"labelCaption":"Контракт отправитель:"
				}),
		...
		,"to_oktmo":{
			"binding":new CommandBinding({
				"control":new OKTMOEdit(id+":filter-ctrl-to_oktmo",{
					"contClassName":"form-group-filter",
					"labelCaption":"ОКТМО получатель:",
					"dependBaseControl":"filter-ctrl-to_oktmo_contract",
					"dependBaseFieldIds":["id"],
					"dependFieldIds":["to_oktmo_id"]
				}),
				"field":new FieldInt("to_oktmo_id")}),
			"sign":"e"		
		}
		,"to_oktmo_contract":{
			"binding":new CommandBinding({
				"control":new OKTMOContractEdit(id+":filter-ctrl-to_oktmo_contract",{
					"contClassName":"form-group-filter",
					"labelCaption":"Контракт получатель:"
				}),
				"field":new FieldInt("to_oktmo_contract_id")}),
			"sign":"e"		
		}
	...
	this.addElement(new GridAjx(id+":grid",{
		...
		"controller":contr,
		"editInline":false,
		"editWinClass":DocMovementDialog_Form,
			...
			"elements":[
				new GridRow(id+":grid:head:row0",{
					"elements":[
						...
						,new GridCellHead(id+":grid:head:to_oktmo_ref",{
							"value":"ОКТМО получатель",
							"columns":[
								new GridColumnRef({
									"field":model.getField("to_oktmo_ref"),
									"ctrlClass":OKTMOEdit,
									"searchOptions":{
										"field":new FieldInt("to_oktmo_id"),
										"searchType":"on_match",
						...
						,new GridCellHead(id+":grid:head:to_oktmo_contracts_ref",{
							"value":"Контракт получатель",
							"columns":[
								new GridColumnRef({
									"field":model.getField("to_oktmo_contracts_ref"),
									"ctrlClass":OKTMOContractEdit,
									"searchOptions":{
										"field":new FieldInt("to_oktmo_contract_id"),
										"searchType":"on_match",
